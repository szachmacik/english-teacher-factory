...
## 17. Spaced Repetition Flashcards (SM-2 Algorithm)

**Pattern:** Implement the SM-2 algorithm for vocabulary learning without any external library.

```typescript
// Pure TypeScript SM-2 — no dependencies needed
function sm2(card: FlashCard, quality: number): FlashCard {
  ...
  const state = repetitions === 0 ? "learning" : interval >= 21 ? "mastered" : "review";
...
**Key insight:** Card flip animation uses CSS `transform: rotateY(180deg)` with `backfaceVisibility: hidden`. Keyboard shortcuts (Space=flip, 1-4=quality) via `useEffect` + `window.addEventListener`.

---

## 18. CSS 3D Card Flip Without Libraries
...
## 19. AI Chat Assistant with Project Context

**Pattern:** Chat assistant that has access to project data (transcript, products, vocabulary) to give contextually relevant teaching suggestions.

```typescript
// server/routers/factory.ts — chat.send procedure
chat: router({
  send: protectedProcedure
    .input(z.object({ projectId: z.number(), message: z.string() }))
    .mutation(async ({ ctx, input }) => {
      const project = await getProjectById(input.projectId);
      const systemPrompt = `You are an expert EFL/ESL teaching assistant.
        Project: "${project.title}" | Level: ${project.cefrLevel}
        Transcript excerpt: ${project.transcript?.slice(0, 500)}...`;
      
      const history = await getChatHistory(input.projectId);
      const messages = [
      ...
      await saveChatMessages(input.projectId, input.message, reply);
      return { reply };
    }),
})
```
...
## 20. Analytics Dashboard with Recharts

**Pattern:** Use `recharts` (already in package.json) for production analytics.

```tsx
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, PieChart, Pie, Cell } from "recharts";

// Products by type bar chart
...
## 21. Teacher Share — Public Game Hub

**Pattern:** Each project gets a public `/project/:id/share` page showing all games with live leaderboards. No auth required for students to play.

- Games use `shareToken` (nanoid) for public access
- Game sessions stored in `game_sessions` table with `studentName`, `score`, `completedAt`
- Teacher sees real-time leaderboard via polling (`refetchInterval: 10000`)
- Share via Web Share API: `navigator.share({ url, title, text })`
...
## 22. Certificate Generator (Print API)

**Pattern:** Generate printable PDF certificates using HTML + `window.print()` — no server needed.
...
- **Bulk processing** up to 20 sources simultaneously
- **Voice recording** in browser via Web Audio API → Whisper
- **SM-2 Spaced Repetition** Flashcard Study mode
- **AI Chat Assistant** with project context
- **Analytics Dashboard** with Recharts charts
- **Teacher Share** public game hub with live leaderboards
- **Certificate Generator** with print API
- **Zero TypeScript errors**

---

*Generated by EduFactory v4.0 — English Teacher Digital Product Factory*
